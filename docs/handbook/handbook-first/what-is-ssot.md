...
近年、企業を取り巻くビジネス環境は複雑化しており、扱われるデータの種類や量も急激に増加している。こうした環境下で、以下のような課題が表面化しやすくなった。
...
本ハンドブックは、グループ運営に関するルールや情報のSSoTとして位置付ける。
同じ情報が複数箇所に存在する状態を避け、判断や行動の拠り所を本ハンドブックに一本化する。

この状態を維持するため、以下を運用の原則とする。

- ルールや情報は本ハンドブックに集約し、他の場所に重複して記載しない
- 参照が必要な場合は、コピーではなくリンクで結ぶ
- 古くなった記述や誤りを見つけたら、気づいた人が[更新](../update-handbook)する
- 変更は[ハンドブックレビュー](../review)を経て、品質と整合性を保つ
